...
export async function POST(req: NextRequest) {
  ...
  let body: { nome: string; dataNascimento: string; email?: string; clube: string; jogadorFavorito: string; peso?: string; altura?: string; fotoBase64: string; retryAttempt?: number; };
  try {
    body = await req.json();
  } catch {
  ...
  const { nome, dataNascimento, email, clube, jogadorFavorito, peso, altura, fotoBase64, retryAttempt } = body;
